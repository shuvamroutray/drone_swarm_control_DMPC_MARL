...
from gym_pybullet_drones.control.DMPCControl_Obstacle import DMPCControl


class DMPCAviary(BaseAviary):
    """Multi-drone environment class for DMPC swarm control."""

    def __init__(self, num_drones: int = 2, target_pos: np.ndarray = np.array([2.0, 0.0, 1.5]),obstacles=None, **kwargs):

        # --- Handle per-drone or single target input ---
        self.NUM_DRONES = num_drones


        # after creating self.dmpc_controllers
        ...
        # store obstacles as list of 3-vectors (world positions)
        if obstacles is None:
            self.OBSTACLE_POSITIONS = []   # empty list -> no obstacles
        else:
            # ensure numpy array list of shape (M,3)
        ...
        # create small visual/collision spheres for obstacles so they show in GUI/replay
        self.OBSTACLE_IDS = []
        if len(self.OBSTACLE_POSITIONS) > 0:
            for idx, pos in enumerate(self.OBSTACLE_POSITIONS):
                pos = np.asarray(pos).reshape(3,)
                # ensure obstacle isn't below ground (raise a bit)
                if pos[2] < 0.05:
                    pos[2] = 0.05

                # choose a visible radius and opaque color
                radius = max(0.06, 0.5 * self.D_SAFE)  # use >= 6cm for visibility
                rgba = [1.0, 0.2, 0.2, 1.0]  # opaque red

                # create visual + collision shapes with the correct client id
                ...
                colShapeId = p.createCollisionShape(shapeType=p.GEOM_SPHERE,
                                                    radius=float(radius),
                                                    physicsClientId=self.CLIENT)
                # create a static multi-body so it shows in GUI
                ...
                # add a white text label above obstacle to make it easy to spot
                try:
            ...
            print(f"[DMPCAviary] created {len(self.OBSTACLE_IDS)} obstacle visuals: positions={self.OBSTACLE_POSITIONS}")
    ...
    def _preprocessAction(self, action):
        ...
        for i in range(self.NUM_DRONES):
            ...
            # --- add static obstacles as additional neighbor blocks ---
            for obs_pos in self.OBSTACLE_POSITIONS:
                # create constant predicted trajectory: (Np+1, 6) with v=0
                block = np.tile(np.hstack([obs_pos.reshape(3,), np.zeros(3,)]), (self.NP + 1, 1)).flatten()
                neighbor_blocks.append(block)

            # flatten (if no neighbors, create empty array)
            ...
            # append to env-level lists (safe even if controller lists are empty)
            try:
            ...
            # optionally capture controller-reported status/iters/object
