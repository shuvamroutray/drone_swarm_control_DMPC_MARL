...
class CentralizedMARLNode(Node):
    ...
    def nearest_frontier(self, gx, gy, other_positions):
        """
        Find nearest unexplored cell globally with collision avoidance.
        """
        best_dist = float("inf")
        best_target = (gx, gy)
    ...
    def update_grid(self):
        ...
        for drone_id, (gx, gy) in self.positions_grid.items():
            ...
            # Reset anomaly roles after resolution
            ax, ay = self.anomaly_pos
            ...
            if len(self.recent_positions[drone_id]) > 12:
    ...
    def run_policy(self):
        ...
        if (
            ...
            and not np.any(self.true_grid == 3)
        ...
        reserved_targets = set()
                # ============================================================
        # FINAL CONSOLIDATED run_policy() LOOP (DROP-IN REPLACEMENT)
        # Replace your ENTIRE per-agent decision section inside:
        ...
        for drone_id in range(self.num_drones):
            ...
            if not self.target_reached[drone_id]:
                ...
                if self.has_reached_target(drone_id):
                    ...
                    # If stuck too long, force replanning
                    if len(self.recent_positions[drone_id]) >= 4:
                        if len(set(self.recent_positions[drone_id][-4:])) <= 2:
                            self.get_logger().info(
                                f"[Agent {drone_id}] Deadlock detected -> replanning."
                            )
                            self.target_reached[drone_id] = True
                    else:
                        continue
            ...
            if self.detected and np.any(self.grid == 3):
                ...
                if drone_id == self.anomaly_discoverer:
                    ...
                    for alt_action, (adx, ady) in ACTIONS.items():
                        ...
                        # Prevent head-on swaps
                        other_current_targets = {
                            self.current_targets[o]
                            for o in range(self.num_drones)
                            if o != drone_id and self.current_targets[o] is not None
                        }

                        if (nx, ny) in other_current_targets:
                            continue

                        # Avoid collisions
                        if (
                            (nx, ny) in other_positions
                            or (nx, ny) in reserved_targets
                        ):
                            continue

                        # HARD anomaly exclusion radius
                        anomaly_dist = abs(nx - ax) + abs(ny - ay)

                        if anomaly_dist <= 2:
                            continue

                        # Strong exploration objective
                        if self.grid[nx, ny] == 0:
                            score = 0
                        elif self.grid[nx, ny] in [1, 4]:
                            score = 25
                        else:
                            score = 50

                        # Anti-oscillation
                        if (nx, ny) in self.recent_positions[drone_id]:
                            score += 8

                        # Avoid immediate reversal
                        if self.last_targets[drone_id] == (nx, ny):
                            score += 6

                        valid_moves.append(
                            (score, alt_action, nx, ny)
                        )
                    ...
                    if valid_moves:
                        ...
                        # Global fallback
                        target_gx, target_gy = self.nearest_frontier(
                            gx,
                            gy,
                            other_positions
                        )
                        ...
                        action_source = "DISCOVERER_GLOBAL"

                # --------------------------------
                # RESOLVER = FORCE CROSSCHECK
                # --------------------------------
                    ...
                    for alt_action, (adx, ady) in ACTIONS.items():
                        ...
                        # Direct shortest path to anomaly
                        dist = abs(nx - ax) + abs(ny - ay)

                        candidates.append(
                            (dist, alt_action, nx, ny)
                        )
                    ...
                    if candidates:
                        ...
                        _, action, target_gx, target_gy = candidates[0]

                    else:
                        target_gx, target_gy = gx, gy
                        action = None

                    action_source = "RESOLVER_CROSSCHECK"
            ...
            # =====================================================
                ...
                marl_action = self.policy.predict(obs)

                dx, dy = ACTIONS[marl_action]

                candidate_gx = np.clip(
                    gx + dx,
                    0,
                    self.grid_size - 1
                )

                candidate_gy = np.clip(
                    gy + dy,
                    0,
                    self.grid_size - 1
                )
                ...
                # --------------------------------
                # Evaluate ALL valid alternatives
                # --------------------------------
                for alt_action, (adx, ady) in ACTIONS.items():
                ...
                # --------------------------------
                if self.grid[candidate_gx, candidate_gy] == 3:
                    marl_score = -20

                elif self.grid[candidate_gx, candidate_gy] == 0:
                    marl_score = 0

                elif self.grid[candidate_gx, candidate_gy] in [1, 4]:
                    marl_score = 25
                ...
                # --------------------------------
                # Global frontier fallback
                # --------------------------------
                if best_score >= 25:

                    frontier_gx, frontier_gy = self.nearest_frontier(
                        gx,
                        gy,
                        other_positions
                    )

                    target_gx, target_gy = frontier_gx, frontier_gy
            ...
            # --------------------------------------------------
            # REALISTIC TARGET REACHED STATE
            # --------------------------------------------------
            if new_target != (gx, gy):
            ...
            self.get_logger().info(
                ...
                f"Recent={self.recent_positions[drone_id]}"
            )

            if valid_moves:
                self.get_logger().info(
                    f"[Agent {drone_id}] "
                    f"TopMoves={valid_moves[:3]}"
                )
