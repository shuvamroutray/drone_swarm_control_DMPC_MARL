...
class DMPCAgent(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"START={np.round(self.start_position,2)} "
            f"GOAL={np.round(self.target,2)}"
        )
        ...
        msg.pose.position.x = self.start_position[0]
        msg.pose.position.y = self.start_position[1]
        msg.pose.position.z = self.start_position[2]
    ...
    #         self.get_logger().info(
    #             f"[{self.drone_name}] New MARL target: {np.round(self.target,2)}"
    #         )
