...
do
    ...
    echo "Waiting for takeoff services..."

    until ros2 service list | grep -q "/cf231/takeoff"
    do
        sleep 1
    done

    until ros2 service list | grep -q "/cf232/takeoff"
    do
        sleep 1
    done

    echo "Takeoff services available"
    ...
    ros2 launch control_swarm_ros \
        swarm_dmpc_benchmark_ThesisResult_2.launch.py \
        scenario_seed:=$seed \
